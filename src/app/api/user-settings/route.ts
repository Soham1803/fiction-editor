import { createUserSettings, getUserSettingsByUserId, updateUserSettings, deleteUserSettings } from '@/db/queries/userSettings';
import { NextRequest, NextResponse } from 'next/server';

export async function POST(req: NextRequest){
  try{
    const { userId, theme, fontSize, notifications_enabled } = await req.json();

    if(!userId || !theme || !fontSize || !notifications_enabled){
      return NextResponse.json({ message: "Please provide all the required fields", status: 400});
    }

    const response = await createUserSettings({ userId, theme, font_size: fontSize, notifications_enabled });
