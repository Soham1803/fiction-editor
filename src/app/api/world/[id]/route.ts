import { updateWorld, deleteWorld } from "@/db/queries/world";
...
export async function PUT(req: NextRequest, params: {id: string}){
    try {
        ...
        const response = await updateWorld(_id, data);

        return NextResponse.json({message: "World updated successfully", response, status: 200})
    } catch (error) {
...
export async function DELETE(req: NextResponse, params: {id: string}) {
    try {
        const _id = params.id;
        const response = await deleteWorld(_id);

        return NextResponse.json({message: "World deleted successfully", response, status: 200})
    } catch (error) {
        console.log("Catched Error: ", error);
        return NextResponse.json({message: "Unexpected error occurred, please check the logs!", status: 500})
    }
